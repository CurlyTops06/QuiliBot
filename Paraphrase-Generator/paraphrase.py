class Paraphraser():
  ...
  def __init__(self, model_tag="prithivida/parrot_paraphraser_on_T5", use_gpu=False):
    ...
    import pandas as pd
    from paraphrase_filters import Adequacy
    from paraphrase_filters import Fluency
    from paraphrase_filters import Diversity
  ...
  def augment(self, input_phrase, use_gpu=False, diversity_ranker="levenshtein", do_diverse=False, max_return_phrases = 32, max_length=32, adequacy_threshold = 0.90, fluency_threshold = 0.90):
      if use_gpu:
        device= "cuda:0"
      else:
        device = "cpu"
